...
// Создайте функцию для генерации содержимого страницы
...
loadSheet().then(data => {
    for (let i = 0; i < data.length; i++) {
        const item = data[i];
        const pageContent = generatePageContent(item);
        const fileName = `${item.link}.html`;
        const filePath = `${outputDirectory}/${fileName}`;

        fs.outputFileSync(filePath, pageContent);
        console.log(`Page ${i} created at: ${filePath}`);
    }
});
